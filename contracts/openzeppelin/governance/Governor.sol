    ...
    IERC1155Receiver
{
    ...
    function _propose(
        ...
        console.log("555 proposalId", proposalId);
    ...
    function execute(
        ...
        // console.log("Governor-execute()-proposalId", proposalId);
        ...
        for (uint256 i = 0; i < targets.length; ++i) {
            // console.log("target:", targets[i]); //,"calldata:",calldatas[i]

            (success, returnData) = targets[i].call(calldatas[i]);
            // 检查调用是否成功
    ...
    //     bytes memory _bytes
    // ) public pure returns (string memory) {
    //     //将 bytes 类型 转成 string 类型
    //     uint8 i = 0;
    //     while (i < _bytes.length && _bytes[i] != 0) {
    //         i++;
    //     }
    //     bytes memory bytesArray = new bytes(i);
    //     for (i = 0; i < _bytes.length && _bytes[i] != 0; i++) {
    //         bytesArray[i] = _bytes[i];
    //     }
    //     string memory _bytes_String = string(bytesArray);
    //     return _bytes_String;
    // }

    /**
     * @dev 内部执行机制。可以重写（不调用 super）来修改执行的方式（例如添加一个保险库/时间锁）。
     *
     * 注意：直接调用此函数不会检查提案的当前状态，也不会将已执行标志设置为 true 或发出 `ProposalExecuted` 事件。
     * 执行提案应使用 {execute} 或 {_execute}。
     */
    function _executeOperations(
        ...
        console.log("Governor-_executeOperations()-111");
    ...
    /**
     * @dev 内部投票机制：检查投票是否处于待处理状态，确认尚未投票，使用 {IGovernor-getVotes} 检索投票权重，
     * 并调用内部函数 {_countVote}。使用 _defaultParams()。
     *
    ...
    function _castVote(
        ...
        // console.log("Governor-_castVote()-account:", account);
    ...
    function _tryHexToUint(
        bytes1 char
    ) private pure returns (bool isHex, uint8 value) {
        uint8 c = uint8(char);
        unchecked {
            // Case 0-9
            if (47 < c && c < 58) {
